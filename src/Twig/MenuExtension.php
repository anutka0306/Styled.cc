...
class MenuExtension extends AbstractExtension
{
    ...
    public function mainMenu(Environment $twig, ?PageInterface $page): string
    {
        //$location = $page->getLocation();
        $location = $page ? $page->getLocation() : null;
        $itemName = $location ? 'main_menu' . '-' . $location->getPath() : 'main_menu';
 
        $item = $this->cache->getItem($itemName);
        if (!$item->isHit()) {//если данное значение не закешировано
            $items = $this->getServiceMenuItems();

            return $twig->render('v2/extensions/mainmenu.html.twig',[
                'items' => $items,
                'location' => $location
            ]);
            $item->set($html);
    ...
    public function offcanvas(Environment $twig,?PageInterface $page): string
    {
        $location = $page->getLocation();
        $location = $page ? $page->getLocation() : null;
        $itemName = $location ? 'offcanvas' . '-' . $location->getPath() : 'offcanvas';
        $item = $this->cache->getItem($itemName);
        if (!$item->isHit()) {//если данное значение не закешировано
            $items = $this->getServiceMenuItems();
